...
/// PlaybackSessionJoinOrCreateNetwork is used to join an existing session in the group, if the appId and appContext match the existing session’s. If there is no current session and no other source is playing, joinOrCreateSession creates a new session and joins it.
...
public class PlaybackSessionJoinOrCreateNetwork: Network {
    ...
    /// Initializes an instance of PlaybackSessionJoinOrCreateNetwork to join an existing session in the group, if the appId and appContext match the existing session’s.
    ...
    public init(accessToken: String, groupId: String, accountId: String?, appContext: String, appId: String, customData: String?, success: @escaping (Data?) -> Void, failure: @escaping (Error?) -> Void) {
    ...
    override func preparedURL() -> URLConvertible {
        return "https://api.ws.sonos.com/control/api/v1/groups/\(groupId)/playbackSession/joinOrCreate"
    }
