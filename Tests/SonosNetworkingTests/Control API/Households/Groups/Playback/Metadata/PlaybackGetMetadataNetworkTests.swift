...
final class PlaybackGetMetadataNetworkTests: XCTestCase {
    private var subject: PlaybackGetMetadataNetwork!
    ...
    override func setUp() {
        subject = PlaybackGetMetadataNetwork(accessToken: MockedData.accessToken,
    ...
    func testParameters() {
        XCTAssertNil(subject.parameters()?.keys)
    }
...
fileprivate final class MockedData {
    ...
    public static let expectedPreparedUrl: URLConvertible = "https://api.ws.sonos.com/control/api/v1/groups/\(groupId)/playbackMetadata"
    ...
    public static let expectedResults: [String: Any] = [
        "container": [
            ...
            "imageUrl": "http://acme.example.com/image/thumb/ABC/400x400image.jpeg"
        ],
        "currentItem": [
            "track": [
                "type": "track",
                "name": "Sample track",
                "imageUrl": "http://0.0.0.0:1400/getaa?ABC123",
                "album": [
                    "name": "Sample album"
                ],
                "artist": [
